...
class trx_addons_demo_data_importer {
	...
	function __construct() {
		...
		if (!function_exists('trx_addons_exists_ocdi') || !trx_addons_exists_ocdi()) {
			...
			add_filter('trx_addons_filter_theme_panel_tabs',			array($this, 'add_theme_panel_tab'));
	...
	function add_theme_panel_step( $steps ) {
		trx_addons_array_insert_after( $steps, 'plugins', array( 'demo' => esc_html__( 'Import demo data.', 'trx_addons' ) ) );
		return $steps;
	}
		...
		?><div class="trx_importer"><?php
			...
			if (is_array($this->options['banners']) && count($this->options['banners']) > 0) {
				...
				foreach ($this->options['banners'] as $banner) {
					...
					if (!empty($banner['content'])) {
						$banners .= '<div class="trx_banners_item_content">' . wp_kses_post($banner['content']) . '</div>';
					}
					// Link
					if (!empty($banner['link_url'])) {
						$banners .= '<a class="trx_banners_item_link' . (empty($banner['link_caption'])	? ' trx_banners_item_link_block' : ' button button-primary') . '"'
					...
					<form id="trx_exporter_form" action="<?php echo !empty($tab_id) ? get_admin_url( null, 'admin.php?page=trx_addons_theme_panel#trx_addons_theme_panel_section_demo' ) : '#'; ?>" method="post">
						...
						if ( isset($_POST['exporter_action']) ) { 
							...
							<div class="trx_buttons">
								<input type="submit" value="<?php esc_attr_e('Export Demo Data', 'trx_addons'); ?>" class="button button-primary">
							</div>
							<?php
						}
						?>
			...
			if ( $this->options['allow_import'] && !isset($_POST['exporter_action']) ) { 
				?><div class="trx_importer_section">
					...
					<h1 class="trx_addons_theme_panel_section_title">
						<?php esc_html_e( 'Import Demo Data', 'trx_addons' ); ?>
					</h1>
					...
					// Banners in the tabs
					if (!empty($tab_id)) {
						trx_addons_show_layout($banners);
					}
					...
					?><form id="trx_importer_form">
						...
						<div class="trx_importer_demo_set">
							...
							<?php
							$msg_long_time   = __('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'trx_addons');
							$msg_take_memory = __('<b>Some plugins</b> (for example BuddyPress and bbPress) <b>increases the memory consumption</b> at 60-80MB depending on the configuration. So if you have problems with installing the demo-data due to lack of memory - deactivate some plugins and then retry the installation of the demo-data.', 'trx_addons');
							?>
							<div class="trx_importer_description trx_importer_description_part<?php if ($this->options['demo_set']!='part') echo ' trx_importer_hidden'; ?>">
								<ol>
									...
									<li><?php echo wp_kses_data($msg_long_time); ?></li>
									<li><?php echo wp_kses_data($msg_take_memory); ?></li>
								</ol>
							</div>
						...
						<div class="trx_buttons">
							<input type="button" value="<?php esc_attr_e('Start Import', 'trx_addons'); ?>" class="button button-action">
						</div>
					</form>
	...
	function exporter() {
		...
		$users['users'] = apply_filters('trx_addons_filter_export_users', $rows);
		...
		$users['usermeta'] = apply_filters('trx_addons_filter_export_usermeta', $rows);
	...
	function import_uploads($uploads = '', $import_type = '') {
		...
		if($uploads == '') $uploads = $this->options['files'][$this->options['demo_type']]['file_with_uploads'];
		...
		foreach ($files as $file) {
			...
			if (trx_addons_get_file_ext($file)=='zip') {
				if ( ($zip = $this->download_file($file, round(max(0, $counter-1) / count($files) * 100), $import_type)) === '')
					$need_exit = true;
				else {
			...
			// Append next part (*.001, *.002 ...) to archive
				...
				if ( ($txt = $this->get_file($file, round(max(0, $counter-1) / count($files) * 100), $import_type)) === false)
					$need_exit = true;
				else {
	...
	function import_dump($slug, $title) {
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug']) 
				dfl(sprintf(__('Unable to unserialize data from the file %s', 'trx_addons'), trx_addons_get_file_name($this->options['files'][$demo_type]['file_with_' . $slug])));
		}
		$this->response['result'] = $result;
	}
	...
	function get_list_pages_from_demo($demo_type) {
		...
		if (!$list || !is_array($list)) {
			...
			set_transient("trx_addons_installer_posts", $list, 10*60);	// Store to cache for 10 minutes
		}
		return $list;
	}
	...
	function replace_site_url($str) {
		...
		$old_url = $this->options['files'][$this->options['demo_type']]['domain_demo'];
		if (substr($old_url, -1)=='/') $old_url = substr($old_url, 0, strlen($old_url)-1);
        return trx_addons_str_replace(array(
            $old_url,
            str_replace('/', '\\/', $old_url),
	...
	function prepare_data($str) {
		...
		if (is_string($str) && is_serialized($str)) {
			...
			$need_ser = is_array($str) || is_object($str);
		}
		if (is_array($str)) {
			foreach ($str as $k=>$v) {
