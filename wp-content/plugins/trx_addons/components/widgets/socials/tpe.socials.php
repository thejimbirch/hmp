...
var socials = settings.type == 'socials'
				? '<?php echo addslashes(trx_addons_get_socials_links()); ?>'
				: '<?php echo addslashes(trx_addons_get_share_links(array(
						'type' => 'block',
						'caption' => '',
						'echo' => false
					))); ?>';
if ( socials != '') {
	#><div class="{{ settings.type }}_wrap sc_align_{{ settings.align }}">{{{ socials }}}</div><#
}
#><?php	
