		...
		<div class="categories_list_item">
			<div class="categories_list_icon">
				<div class="wrap_in">
					<?php
					if ($show_thumbs) {
						$icon = trx_addons_get_term_image_small($cat->term_id, $cat->taxonomy);
						if (!empty($icon)) {
							$src = trx_addons_add_thumb_size($icon, trx_addons_get_thumb_size('tiny'));
							$attr = trx_addons_getimagesize($src);
							?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category icon', 'trx_addons'); ?>"><?php
						}
					}
