...
?>><?php
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			?><div class="sc_services_item_marker sc_services_item_thumb" style="background-image: url(<?php
				$trx_addons_attachment_src = wp_get_attachment_image_src(
														...
														apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('masonry'), 'services-timeline')
														);
			...
			echo !empty($link) 
				? '<a href="'.esc_url($link).'"'.(!empty($meta['link']) ? ' target="_blank"' : '') 
				: '<span';
			?> class="sc_services_item_marker sc_services_item_pictogram"><?php
	...
	<div class="sc_services_item_info">
		...
		if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']!='image')) {
			do_action( 'trx_addons_action_services_item_before_header_featured', $args );
			...
			$price_showed = true;
			do_action( 'trx_addons_action_services_item_after_header_featured', $args );
		}
		?>
