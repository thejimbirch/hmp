...
if ( !function_exists( 'trx_addons_cpt_properties_load_responsive_styles' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_cpt_properties_load_responsive_styles', 2000);
	function trx_addons_cpt_properties_load_responsive_styles() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-cpt_properties-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'properties/properties.responsive.css'), array(), null );
		}
	}
}
...
if ( !function_exists( 'trx_addons_cpt_properties_merge_styles_responsive' ) ) {
	...
	function trx_addons_cpt_properties_merge_styles_responsive($list) {
		$list[] = TRX_ADDONS_PLUGIN_CPT . 'properties/properties.responsive.css';
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_cpt_properties_related_posts_show' ) ) {
	...
	function trx_addons_cpt_properties_related_posts_show( $show ) {
		if (!$show && is_single() && get_post_type() == TRX_ADDONS_CPT_PROPERTIES_PT) {
