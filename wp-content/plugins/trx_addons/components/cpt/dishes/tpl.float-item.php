...
?>
<div data-post-id="<?php the_ID(); ?>" class="sc_dishes_item with_image<?php
	...
	<div class="sc_dishes_item_header">
		<h5 class="sc_dishes_item_title entry-title<?php if (!$price_showed && trim($meta['price']) != '') echo ' with_price'; ?>"><a href="<?php echo esc_url($link); ?>"><?php
			the_title();
			// Price
	...
	if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) {
		...
		if (!empty($args['more_text']) || $meta['product'] > 0) {
