...
// Check if plugin is in frontend editor mode
...
if ( !function_exists( 'trx_addons_vc_edit_form_end' ) ) {
	...
	function trx_addons_vc_edit_form_end() {
...
// Load required scripts to the VC Edit form
...
if ( !function_exists( 'trx_addons_vc_load_scripts_front' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_vc_load_scripts_front', 11);
...
if ( !function_exists( 'trx_addons_vc_load_responsive_styles' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_vc_load_responsive_styles', 2000);
	function trx_addons_vc_load_responsive_styles() {
		if ( trx_addons_exists_vc() && trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-js_composer-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_API . 'js_composer/js_composer.responsive.css'), array(), null );
		}
	}
}
...
if ( !function_exists( 'trx_addons_vc_merge_styles_responsive' ) ) {
	...
	function trx_addons_vc_merge_styles_responsive($list) {
		if (trx_addons_exists_vc())
			$list[] = TRX_ADDONS_PLUGIN_API . 'js_composer/js_composer.responsive.css';
		return $list;
	}
}
...
// Add/Remove standard VC shortcodes params
	...
	function trx_addons_vc_add_params() {
		...
		if (trx_addons_exists_vc()) {
			...
			if (!empty($TRX_ADDONS_STORAGE['shapes_list'])) {
				$shapes = array(
					'none' => esc_html__('- Not selected -', 'trx_addons')
				);
...
if ( !function_exists( 'trx_addons_vc_add_params_classes' ) ) {
	if (trx_addons_exists_vc()) add_filter( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'trx_addons_vc_add_params_classes', 10, 3 );
...
// Prepare VC shortcodes html output
		...
		if (in_array($sc, array('vc_row', 'vc_row_inner'))) {
			foreach (array('top', 'bottom') as $side) {
				...
				if (preg_match($reg_exp, $html, $matches) && !empty($matches[2]) && !trx_addons_is_off($matches[2])) {
					...
					$shape_dir = trx_addons_get_file_dir("css/shapes/{$shape_name}.svg");
					if (!empty($shape_dir)) {
...
// OCDI support
if ( is_admin() && trx_addons_exists_vc() && trx_addons_exists_ocdi() ) {
