	...
	function trx_addons_get_share_links($args, $list=false) {
		...
		if (is_array($list)) {
			foreach ($list as $social) {
				$sn = $social['name'];
				$fn = $args['style']=='icons' ? trx_addons_clear_icon_name($sn) : trx_addons_get_file_name($sn);
				...
				$output .= '>'
							. '<span class="social_icon social_icon_' . esc_attr($fn) . ' sc_icon_type_' . esc_attr($args['style']) . '"'
...
if (!function_exists('trx_addons_add_og_tags')) {
	...
	function trx_addons_add_og_tags() {
		...
		if (is_singular() && (!isset($wp_query->is_posts_page) || $wp_query->is_posts_page!=1) && !empty($wp_query->post)) {
			?><meta property="og:url" content="<?php echo esc_url(get_permalink()); ?> "/>
			<meta property="og:title" content="<?php echo esc_attr( strip_tags( get_the_title() ) ); ?>" />
			<meta property="og:description" content="<?php echo esc_attr( strip_tags( strip_shortcodes( trx_addons_get_post_excerpt() ) ) ); ?>" />  
