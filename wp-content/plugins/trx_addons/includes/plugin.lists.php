		...
		if ($list === false) {
			...
			if (is_array($post_types)) {
				foreach ($post_types as $pt) {
					$terms = get_object_taxonomies( $pt->name, array( 'hide_empty' => false ) );
					foreach ($terms as $t) {
...
if ( !function_exists( 'trx_addons_get_list_sc_share_types' ) ) {
	function trx_addons_get_list_sc_share_types() {
		return apply_filters('trx_addons_filter_get_list_sc_share_types', array(
				...
				'block' => __( 'Separate icons', 'trx_addons' ),
			));
	}
}
...
if ( !function_exists( 'trx_addons_get_list_sc_image_ratio' ) ) {
	function trx_addons_get_list_sc_image_ratio() {
		return apply_filters('trx_addons_filter_get_list_sc_image_ratio', array(
...
// Return list of the yandexmap styles
if ( !function_exists( 'trx_addons_get_list_sc_yandexmap_styles' ) ) {
	function trx_addons_get_list_sc_yandexmap_styles() {
		return apply_filters('trx_addons_filter_sc_yandexmap_styles', array(
			'default' => esc_html__('Default', 'trx_addons'),
			'greyscale' => esc_html__('Greyscale', 'trx_addons'),
			'inverse' => esc_html__('Inverse', 'trx_addons'),
		));
...
// Return list of the popup positions
