...
jQuery( document ).ready( function() {
	...
	var need_update = false,
		update_buttons = jQuery( '.trx_updater_upgrade_button,.trx_updater_backups_button,#upgrade,#upgrade-plugins,#upgrade-plugins-2,#upgrade-themes,#upgrade-themes-2' );
				...
				if ( checked.length > 0 ) {
					if ( action == 'update' || need_update !== false || confirm( TRX_UPDATER_STORAGE['msg_options_' + action] + ( action == 'delete' ? "\n" + TRX_UPDATER_STORAGE['msg_irreversable'] : '' ) ) ) {
						if ( need_update === false ) {
							...
							update_buttons.attr( 'disabled', 'disabled' );
