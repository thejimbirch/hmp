...
while ( have_posts() ) { the_post();
	...
	<article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>" <?php post_class( 'services_single itemscope' ); trx_addons_seo_snippets('', 'Article'); ?>>
		...
		<section class="services_page_header">	
            ...
            if ( plumbing_is_off( plumbing_get_theme_option( 'hide_featured_on_single_services' ) ) ) {
		...
		if (!empty($meta['product']) && (int) $meta['product'] > 0) {
			?><div class="services_page_buttons">
				<a href="<?php the_permalink( $meta['product'] ); ?>" class="sc_button theme_button"><?php esc_html_e('Order now', 'plumbing'); ?></a>
			</div><?php
		}
	...
	if ($trx_addons_add_contacts) {
		?><section id="services_page_tab_contacts" class="services_page_section services_page_section_contacts"><?php
			...
			if ( (int) $trx_addons_form_id > 0 ) {
				...
				if ( !function_exists( 'plumbing_cpt_services_wpcf7_form_elements' ) ) {
					add_filter('wpcf7_form_elements', 'plumbing_cpt_services_wpcf7_form_elements');
					function plumbing_cpt_services_wpcf7_form_elements($elements) {
						$elements = str_replace('</textarea>',
												esc_html(sprintf(__("Hi.\nI'd like this service '%s'.\nPlease, get in touch with me.", 'plumbing'), get_the_title()))
												. '</textarea>',
												$elements
												);
						return $elements;
