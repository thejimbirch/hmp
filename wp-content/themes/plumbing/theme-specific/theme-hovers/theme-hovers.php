...
if ( ! function_exists( 'plumbing_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'plumbing_hovers_theme_setup3', 3 );
	function plumbing_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		plumbing_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'plumbing' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'plumbing' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'plumbing' ),
			
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'plumbing' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'plumbing' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'plumbing' ),
					),
					'options'  => plumbing_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'plumbing_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'plumbing_hovers_theme_setup9', 9 );
	function plumbing_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'plumbing_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'plumbing_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'plumbing_hovers_responsive_styles', 2000 );
		add_filter( 'plumbing_filter_localize_script', 'plumbing_hovers_localize_script' );
		add_filter( 'plumbing_filter_merge_scripts', 'plumbing_hovers_merge_scripts' );
		add_filter( 'plumbing_filter_merge_styles', 'plumbing_hovers_merge_styles' );
		add_filter( 'plumbing_filter_merge_styles_responsive', 'plumbing_hovers_merge_styles_responsive' );
		add_filter( 'plumbing_filter_get_css', 'plumbing_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'plumbing_hovers_frontend_scripts' ) ) {
	
	function plumbing_hovers_frontend_scripts() {
		if ( plumbing_is_on( plumbing_get_theme_option( 'debug_mode' ) ) ) {
			$plumbing_url = plumbing_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $plumbing_url ) {
				wp_enqueue_script( 'plumbing-hovers', $plumbing_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'plumbing_hovers_frontend_styles' ) ) {
	
	function plumbing_hovers_frontend_styles() {
		if ( plumbing_is_on( plumbing_get_theme_option( 'debug_mode' ) ) ) {
			$plumbing_url = plumbing_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $plumbing_url ) {
				wp_enqueue_style( 'plumbing-hovers', $plumbing_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'plumbing_hovers_responsive_styles' ) ) {
	
	function plumbing_hovers_responsive_styles() {
		if ( plumbing_is_on( plumbing_get_theme_option( 'debug_mode' ) ) ) {
			$plumbing_url = plumbing_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $plumbing_url ) {
				wp_enqueue_style( 'plumbing-hovers-responsive', $plumbing_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'plumbing_hovers_merge_styles' ) ) {
	
	function plumbing_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'plumbing_hovers_merge_styles_responsive' ) ) {
	
	function plumbing_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'plumbing_hovers_localize_script' ) ) {
	
	function plumbing_hovers_localize_script( $arr ) {
		$arr['button_hover'] = plumbing_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'plumbing_hovers_merge_scripts' ) ) {
	
	function plumbing_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'plumbing_hovers_add_icons' ) ) {
	function plumbing_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php plumbing_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'plumbing' ) : esc_html__( 'Add to cart', 'plumbing' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'plumbing' );
					} else {
						esc_html_e( ' ', 'plumbing' );
					}
				}
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php plumbing_show_layout($target); ?> aria-hidden="true" class="icon-search-1"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php plumbing_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php plumbing_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php plumbing_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'plumbing_hovers_get_css' ) ) {
	
	function plumbing_hovers_get_css( $css, $args ) {
...
.post_featured.hover_icon a:not(.slide_link):hover {
...
.woocommerce ul.products .product-category .post_item .post_featured.hover_shop_buttons .icons a.shop_link {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
.woocommerce ul.products .product-category .post_item .post_featured.hover_shop_buttons .icons a.shop_link:hover {
	color: {$colors['inverse_hover']};
