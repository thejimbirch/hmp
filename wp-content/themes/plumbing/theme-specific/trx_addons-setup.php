...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'plumbing_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'plumbing_trx_addons_theme_specific_components' );
	function plumbing_trx_addons_theme_specific_components( $enable = false ) {
		return PLUMBING_THEME_FREE
					? false     // Free version
					: false;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'plumbing_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'plumbing_trx_addons_theme_specific_setup1', 1 );
	function plumbing_trx_addons_theme_specific_setup1() {
		if ( plumbing_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'plumbing_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'plumbing_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'plumbing_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'plumbing_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'plumbing_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'plumbing_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'plumbing_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'plumbing_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'plumbing_trx_addons_theme_specific_setup' );
	function plumbing_trx_addons_theme_specific_setup() {
		if ( plumbing_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'plumbing_action_before_post_meta', 'plumbing_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'plumbing_trx_addons_cv_enable' ) ) {
	
	function plumbing_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'plumbing_trx_addons_demo_enable' ) ) {
	
	function plumbing_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'plumbing_trx_addons_edd_themes_market_enable' ) ) {
	
	function plumbing_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'plumbing_trx_addons_api_list' ) ) {
	
	function plumbing_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'plumbing_trx_addons_cpt_list' ) ) {
	
	function plumbing_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'plumbing_trx_addons_sc_list' ) ) {
	
	function plumbing_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'plumbing_trx_addons_widgets_list' ) ) {
	
	function plumbing_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'plumbing_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'plumbing_trx_addons_menu_cache' );
	function plumbing_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'plumbing_trx_addons_localize_script' ) ) {
	...
	function plumbing_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = plumbing_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'plumbing_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'plumbing_trx_addons_sc_type', 10, 2 );
	function plumbing_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = plumbing_array_merge( $list, plumbing_get_list_blog_styles( false, 'sc' ) );
		}

		// new type sc_button
		if ($sc == 'trx_sc_button') {
			$list['bordered fullwidth'] = 'Bordered Fullwidth';
		}
...
if ( ! function_exists( 'plumbing_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'plumbing_trx_addons_sc_prepare_atts', 10, 2 );
	function plumbing_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = plumbing_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = plumbing_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'plumbing_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'plumbing_trx_addons_sc_atts', 10, 2 );
	function plumbing_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'plumbing_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'plumbing_trx_addons_sc_map', 10, 2 );
	function plumbing_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'plumbing' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'plumbing' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( plumbing_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'plumbing' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'plumbing' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( plumbing_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'plumbing_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'plumbing_trx_addons_sc_output', 10, 4 );
	function plumbing_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! plumbing_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'plumbing_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'plumbing_trx_addons_sc_item_link_classes', 10, 3 );
	function plumbing_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! plumbing_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'plumbing_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'plumbing_trx_addons_sc_item_title_tag' );
	function plumbing_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'plumbing_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'plumbing_trx_addons_sc_item_button_args', 10, 3 );
	function plumbing_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'plumbing_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'plumbing_trx_addons_slider_title', 10, 2 );
	function plumbing_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'plumbing_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'plumbing_trx_addons_sc_googlemap_styles' );
	function plumbing_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'plumbing' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'plumbing_trx_addons_action_before_post_meta' ) ) {
	
	function plumbing_trx_addons_action_before_post_meta() {
		if ( plumbing_exists_trx_addons() ) {
...
if ( ! function_exists( 'plumbing_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'plumbing_trx_addons_tiny_mce_style_formats' );
	function plumbing_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'plumbing' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'plumbing' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'plumbing_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'plumbing_filter_allow_override_header_image', 'plumbing_trx_addons_allow_override_header_image' );
	function plumbing_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					plumbing_is_team_page()
					|| plumbing_is_cars_page()
					|| plumbing_is_cars_agents_page()
					|| plumbing_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'plumbing_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'plumbing_trx_addons_thumb_size', 10, 2 );
	function plumbing_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
if( !function_exists('plumbing_remove_parent_filters') ) {
	add_filter( 'trx_addons_filter_options', 'plumbing_remove_parent_filters', 10, 1 );
	function plumbing_remove_parent_filters($list){ 
		unset($list['emotions_info']);
		unset($list['emotions']);
		$list['emotions_allowed']['type'] = 'hidden';
		return $list;
	}
}

if( !function_exists('remove_parent_input_hover') ) {
	add_filter( 'trx_addons_filter_get_list_input_hover', 'remove_parent_input_hover', 10, 1 );
	function remove_parent_input_hover($list){ 
		unset($list['path']);
		unset($list['jump']);
