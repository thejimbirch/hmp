...
if ( ! function_exists( 'plumbing_need_page_title' ) ) {
	function plumbing_need_page_title() {
		return ! is_front_page() && apply_filters( 'plumbing_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'plumbing_show_layout' ) ) {
	function plumbing_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'plumbing_get_logo_image' ) ) {
	function plumbing_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = plumbing_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = plumbing_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = plumbing_is_on( plumbing_get_theme_option( 'logo_retina_enabled' ) )
						? plumbing_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? plumbing_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? plumbing_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'plumbing_get_header_video' ) ) {
	function plumbing_get_header_video() {
		$video = '';
		if ( apply_filters( 'plumbing_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( plumbing_check_theme_option( 'header_video' ) ) {
				$video = plumbing_get_theme_option( 'header_video' );
...
// Show post banner
if ( ! function_exists( 'plumbing_show_post_banner' ) ) {
	function plumbing_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code   = plumbing_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img    = plumbing_get_theme_option( $banner_pos . '_banner_img' );
			$banner_height = plumbing_get_theme_option( $banner_pos . '_banner_height' );
			$banner_class  = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' )
									. plumbing_add_inline_css_class( 
										'background-image:url(' . esc_url( $banner_img ) . ');'
										. ( ! empty( $banner_height )
											? 'min-height:' . plumbing_prepare_css_value( $banner_height ) . ';'
											: ''
											)
										)
								: '';
			$banner_link   = plumbing_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				plumbing_show_layout( wp_kses( $banner_code ), 'plumbing_kses_content'  );
...
if ( ! function_exists( 'plumbing_show_post_featured_image' ) ) {
	function plumbing_show_post_featured_image() {
		$seo = plumbing_is_on( plumbing_get_theme_option( 'seo_snippets' ) );
		// Featured image
		if ( plumbing_is_off( plumbing_get_theme_option( 'hide_featured_on_single' ) )
					&& ! plumbing_sc_layouts_showed( 'featured' )
					&& strpos( plumbing_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'plumbing_action_before_post_featured' );
			plumbing_show_post_featured(
				array(
					'singular' => true,
					'thumb_bg' => plumbing_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth'
				)
			);
			do_action( 'plumbing_action_after_post_featured' );
...
if ( ! function_exists( 'plumbing_show_post_title_and_meta' ) ) {
	function plumbing_show_post_title_and_meta() {

		$seo = plumbing_is_on( plumbing_get_theme_option( 'seo_snippets' ) );

		// Title and post meta
		if ( ( ! plumbing_sc_layouts_showed( 'title' ) || ! plumbing_sc_layouts_showed( 'postmeta' ) ) ) {
			do_action( 'plumbing_action_before_post_title' );
			$need_content_wrap = plumbing_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' && plumbing_get_theme_option( 'post_header_position' ) !== 'on_thumb';
			?>
			<div class="post_header post_header_single entry-header">
				...
				// Post meta
				$meta_components = plumbing_array_get_keys_by_value( plumbing_get_theme_option( 'meta_parts' ) );
				...
				if ( $show_categories && ! plumbing_sc_layouts_showed( 'postmeta' ) && plumbing_is_on( plumbing_get_theme_option( 'show_post_meta' ) ) ) {
					plumbing_show_post_meta(
						apply_filters(
							'plumbing_filter_post_meta_args',
							array(
				...
				// Post title
				if ( ! plumbing_sc_layouts_showed( 'title' ) ) {
					the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
				}
				// Post subtitle
				$post_subtitle = plumbing_get_theme_option( 'post_subtitle' );
				if ( ! empty( $post_subtitle ) ) {
					?>
					<div class="post_subtitle">
						<?php plumbing_show_layout( $post_subtitle ); ?>
					</div>
					<?php
				}
				// Post meta
				if ( ! plumbing_sc_layouts_showed( 'postmeta' ) && plumbing_is_on( plumbing_get_theme_option( 'show_post_meta' ) ) ) {
					plumbing_show_post_meta(
						apply_filters(
							'plumbing_filter_post_meta_args',
							array(
			...
			<?php
			do_action( 'plumbing_action_after_post_title' );
		}
	}
}
...
if ( ! function_exists( 'plumbing_show_post_content' ) ) {
	function plumbing_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$plain = true;
		...
		if ( has_excerpt() ) {
			...
			plumbing_show_layout( plumbing_filter_post_content( get_the_content('') ) );
			...
			$quote = plumbing_get_tag( plumbing_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				plumbing_show_layout( wpautop( $quote ) );
				$plain = false;
			} else {
				plumbing_show_layout( plumbing_filter_post_content( get_the_content() ) );
			}
			...
			if ( $plain ) {
				$len = ! empty( $args['hide_excerpt'] )
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: plumbing_get_theme_option( 'excerpt_length' )
								);
				$output = plumbing_excerpt( $output, $len );
			}
		}
		plumbing_show_layout( $output, $otag, $ctag);
	}
}


// Show post link 'Read more' in the blog posts
if ( ! function_exists( 'plumbing_show_post_more_link' ) ) {
	function plumbing_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
		plumbing_show_layout(
			'<a class="more-link" href="' . esc_url( get_permalink() ) . '">'
				. ( ! empty( $args['more_text'] )
						? esc_html( $args['more_text'] )
						: esc_html__( 'Read more', 'plumbing' )
						)
			. '</a>',
			$otag,
			$ctag
...
if ( ! function_exists( 'plumbing_show_post_meta' ) ) {
	function plumbing_show_post_meta( $args = array() ) {
		if ( is_single() && plumbing_is_off( plumbing_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ' ' ) : apply_filters( 'plumbing_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						plumbing_show_layout( $cats, '<span class="post_meta_item post_categories">', '</span> ');
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'plumbing_filter_get_post_date', plumbing_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
					if ( ! empty( $dt ) ) {
						plumbing_show_layout(
							$dt,
					...
					if ( $author_id > 0 ) {
						...
						$author_avatar = get_avatar( get_the_author_meta( 'user_email', $author_id ), 32 * plumbing_get_retina_multiplier() );
						echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
								. ( plumbing_get_theme_setting( 'show_author_avatar' )
					...
					if ( !is_singular() || have_comments() || comments_open() ) {
						...
						echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-empty">'
								. '<span class="post_meta_number">' . esc_html( $post_comments ) . '</span>'
								. '<span class="post_meta_label">' . ( ($post_comments === 1) ? esc_html__('Comment', 'plumbing') :  esc_html__('Comments', 'plumbing') ) . '</span>'
							. '</a> ';
					}
					...
					if ( function_exists( 'trx_addons_get_post_views' ) ) {
						...
						echo ( is_singular()
							...
							. ' class="post_meta_item post_meta_views trx_addons_icon-eye">'
								...
								. '<span class="post_meta_label">' . ( ($post_views === 1) ? esc_html__('View', 'plumbing') :  esc_html__('Views', 'plumbing') ) . '</span>'
							. ( is_singular()
								? '</span> '
								: '</a> '
								);
				...
				// Likes (Emotions)
					...
					if ( function_exists( 'trx_addons_get_post_likes' ) ) {
						...
						echo ( true == $emotions_allowed
								...
								: '<a href="#"'
									...
									. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'plumbing') : esc_attr__( 'Dislike', 'plumbing' ) ) . '"'
									. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
									. ' data-likes="' . esc_attr( $post_likes ) . '"'
									. ' data-title-like="' . esc_attr__( 'Like', 'plumbing') . '"'
									. ' data-title-dislike="' . esc_attr__( 'Dislike', 'plumbing' ) . '"'
									. '>'
								)
									. '<span class="post_meta_number">' . esc_html($post_likes) . '</span>'
									. '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? ( ($post_likes === 1) ? esc_html__('Reaction', 'plumbing') :  esc_html__('Reactions', 'plumbing') )
											: ( ($post_likes === 1) ? esc_html__('Like', 'plumbing') :  esc_html__('Likes', 'plumbing') )
											)
									. '</span>'
								. '</a> ';
					}
					...
					plumbing_show_share_links(
						array(
							...
							'caption' => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'plumbing' ) : '',
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'plumbing' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

				} else {
					// Custom counter
					do_action( 'plumbing_action_show_post_meta', $comp, get_the_ID(), $args );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'plumbing_show_post_featured' ) ) {
	function plumbing_show_post_featured( $args = array() ) {

		$args = array_merge(
			array(
				'hover'         => plumbing_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: plumbing_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? plumbing_get_no_image( '', true ) : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = plumbing_get_post_image();
				if ( ! empty( $image ) ) {
					$image = plumbing_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . plumbing_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
		...
		if ( ! empty( $args['singular'] ) ) {
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						plumbing_is_on( plumbing_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( plumbing_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				$output .= '>';
				plumbing_show_layout( $output );
				if ( has_post_thumbnail() && $args['seo'] ) {
					$plumbing_attr = plumbing_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $plumbing_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $plumbing_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = plumbing_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						plumbing_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) ) ) {
					$post_content = plumbing_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format ) {
						$video = plumbing_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = plumbing_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = plumbing_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = plumbing_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = plumbing_make_video_autoplay( $video );
								?>
						...
						$audio = plumbing_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = plumbing_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = plumbing_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_author = plumbing_get_tag_attrib( $audio, '<audio>', 'data-author' );
									$media_title  = plumbing_get_tag_attrib( $audio, '<audio>', 'data-caption' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( plumbing_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_author = strip_tags( plumbing_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
										$media_title  = strip_tags( plumbing_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

									}
								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php plumbing_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php plumbing_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								plumbing_show_layout( $audio );
								?>
							</div>
							<?php
						}
				...
				plumbing_show_layout( $post_info );
...
if ( ! function_exists( 'plumbing_get_no_image' ) ) {
	function plumbing_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = plumbing_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || plumbing_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = plumbing_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'plumbing_add_bg_in_post_nav' ) ) {
	function plumbing_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), plumbing_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), plumbing_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'plumbing-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'plumbing_show_related_posts' ) ) {
	function plumbing_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : plumbing_get_post_type_taxonomy();
		...
		if ( 'post' == $args['post_type'] ) {
			$args['tax_query'] = array(
				...
				'field' => 'slug',
				'terms' => array( 'post-format-quote', 'post-format-audio', ),
				'operator' => 'NOT IN',
				)
			);
		}
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) plumbing_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = plumbing_get_theme_option( 'related_position' );
			$related_style = plumbing_get_theme_option( 'related_style' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						...
						esc_html_e( 'You May Also Like', 'plumbing' );
					}
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = plumbing_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = plumbing_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = plumbing_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'plumbing_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					plumbing_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						get_template_part( apply_filters( 'plumbing_filter_get_template_part', 'templates/related-posts', $style ), $style );
						?></div><?php
					}
				?></div><?php		
				if ( $args['slider'] ) {
					plumbing_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'plumbing_show_related_posts_callback' ) ) {
	add_action( 'plumbing_action_related_posts', 'plumbing_show_related_posts_callback' );
	function plumbing_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'plumbing_filter_show_related_posts', false ) ) {
			$plumbing_related_posts   = (int) plumbing_get_theme_option( 'related_posts' );
			$plumbing_related_columns = (int) plumbing_get_theme_option( 'related_columns' );
			$plumbing_related_style   = plumbing_get_theme_option( 'related_style' );
			if ( (int) plumbing_get_theme_option( 'show_related_posts' ) && $plumbing_related_posts > 0 ) {
				plumbing_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $plumbing_related_posts ) ),
						'columns'        => max( 1, min( 6, $plumbing_related_posts, $plumbing_related_columns ) ),
					),
					$plumbing_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'plumbing_show_portfolio_posts' ) ) {
	function plumbing_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? plumbing_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = plumbing_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = plumbing_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$plumbing_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the plumbing_ajax_get_posts action
if ( ! function_exists( 'plumbing_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_plumbing_ajax_get_posts', 'plumbing_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_plumbing_ajax_get_posts', 'plumbing_ajax_get_posts_callback' );
	function plumbing_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'plumbing' );
		}
...
// Show pagination
if ( ! function_exists( 'plumbing_show_pagination' ) ) {
	function plumbing_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = plumbing_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_next'    => false,
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'plumbing' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'plumbing' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'plumbing' ) : esc_html__( 'Newest posts', 'plumbing' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'plumbing' ) : esc_html__( 'Older posts', 'plumbing' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'plumbing_single_comments_field' ) ) {
	function plumbing_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'plumbing_kses_content'  ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'plumbing_kses_content'  ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
