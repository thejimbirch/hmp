	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="plumbing_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.plumbing_post_editor' );
					if ( link.length > 0 ) {
						plumbing_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.plumbing_post_editor', function(e) {
					plumbing_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('plumbing_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function plumbing_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
