...
if ( plumbing_is_on( plumbing_get_theme_option( 'seo_snippets' ) ) ) {
	...
	<div class="structured_data_snippets">
		...
		<div itemscope itemprop="publisher" itemtype="//schema.org/Organization">
			...
			<?php
			$plumbing_logo_image = plumbing_get_logo_image();
			if ( ! empty( $plumbing_logo_image['logo'] ) ) {
				?>
				<meta itemprop="logo" itemtype="//schema.org/ImageObject" content="<?php echo esc_url( $plumbing_logo_image['logo'] ); ?>">
				<?php
			}
			?>
		</div>
		<?php
		if ( plumbing_get_theme_option( 'show_author_info' ) != 1 || ! is_single() || is_attachment() || ! get_the_author_meta( 'description' ) ) {  
			?>
