<div class="front_page_section front_page_section_team<?php
	$plumbing_scheme = plumbing_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $plumbing_scheme ) && ! plumbing_is_inherit( $plumbing_scheme ) ) {
		echo ' scheme_' . esc_attr( $plumbing_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( plumbing_get_theme_option( 'front_page_team_paddings' ) );
?>"
		<?php
		$plumbing_css      = '';
		$plumbing_bg_image = plumbing_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$plumbing_anchor_icon = plumbing_get_theme_option( 'front_page_team_anchor_icon' );
	$plumbing_anchor_text = plumbing_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( plumbing_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$plumbing_css      = '';
			$plumbing_bg_mask  = plumbing_get_theme_option( 'front_page_team_bg_mask' );
			$plumbing_bg_color_type = plumbing_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $plumbing_bg_color_type ) {
				$plumbing_bg_color = plumbing_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$plumbing_caption = plumbing_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $plumbing_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $plumbing_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $plumbing_caption, 'plumbing_kses_content'  ); ?></h2>
				<?php
			}

			// Description (text)
			$plumbing_description = plumbing_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $plumbing_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {v
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $plumbing_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $plumbing_description ), 'plumbing_kses_content'  ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
				...
				if ( ! plumbing_exists_trx_addons() ) {
					...
					plumbing_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
				}
			}
			?>
			</div>
